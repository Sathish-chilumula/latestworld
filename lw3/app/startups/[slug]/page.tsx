...
import { getStartupItem, getStartupNews, type StartupNews } from '@/lib/supabase'
...
export default function StartupDetailPage() {
  ...
  const [related, setRelated] = useState<StartupNews[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    if (!params?.slug) return
    Promise.all([getStartupItem(params.slug as string), getStartupNews(8)]).then(([a, all]) => {
      setArticle(a)
