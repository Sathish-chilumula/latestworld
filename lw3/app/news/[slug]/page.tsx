...
import { getNewsItem, getTechNews, type TechNews } from '@/lib/supabase'
...
export default function NewsDetailPage() {
  ...
  const [related, setRelated] = useState<TechNews[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    if (!params?.slug) return
    Promise.all([getNewsItem(params.slug as string), getTechNews(8)]).then(([a, all]) => {
      setArticle(a)
      setRelated(all.filter(x => x.slug !== params.slug).slice(0, 3))
    }).finally(() => setLoading(false))
  }, [params])

  if (loading) return <div style={{ padding: '4rem', textAlign: 'center', fontFamily: 'Poppins', color: 'var(--muted)' }}>Loading…</div>
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.25rem' }}>
      ...
      <div style={{ background: 'var(--bg-soft)', borderRadius: 12, padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <a href={article.source_url} target="_blank" rel="noopener noreferrer" className="btn-glow">Read Full Story →</a>
      </div>
