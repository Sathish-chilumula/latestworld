...
import { getGithubProject, getGithubProjects, type GithubProject } from '@/lib/supabase'
...
export default function GithubDetailPage() {
  ...
  const [related, setRelated] = useState<GithubProject[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    if (!params?.slug) return
    Promise.all([getGithubProject(params.slug as string), getGithubProjects(8)]).then(([r, all]) => {
      setRepo(r)
      setRelated(all.filter(x => x.slug !== params.slug).slice(0, 4))
