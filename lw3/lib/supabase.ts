...
export async function getCryptoCoins(limit = 50) {
  const { data, error } = await supabase.from('crypto_coins').select('*').order('market_cap_rank', { ascending: true }).limit(limit)
  if (error) { console.error(error); return [] }
  return (data || []) as CryptoCoin[]
}
export async function getCryptoCoin(slug: string) {
  const { data, error } = await supabase.from('crypto_coins').select('*').eq('slug', slug).single()
...
export async function getAllSlugs() {
  const [crypto, news, github, aitools, jobs, startups] = await Promise.all([
    supabase.from('crypto_coins').select('slug,updated_at'),
    ...
    supabase.from('jobs').select('slug,created_at'),
