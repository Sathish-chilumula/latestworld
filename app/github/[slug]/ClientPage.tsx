  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setRepo(fetchedRepo || null)
        setRelated((allRepos || []).filter(x => x.slug !== slug).slice(0, 4))
      } catch (error) {
  ...
  if (loading) {
    return (
      <div style={{ padding: '4rem', textAlign: 'center', fontFamily: 'Poppins', color: 'var(--muted)' }}>
        <div className="live-dot" style={{ display: 'inline-block', marginRight: 10 }} /> Loading project...
      </div>
    )
  }
