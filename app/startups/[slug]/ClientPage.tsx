  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setRelated((allNews || []).filter(x => x.slug !== slug).slice(0, 3))
      } catch (error) {
        console.error(`[Startups] Error fetching data for slug "${slug}":`, error)
      } finally {
        setLoading(false)
      }
    }
    if (slug && slug !== 'placeholder') loadData()
  ...
  if (loading) {
    return (
      <div style={{ padding: '4rem', textAlign: 'center', fontFamily: 'Poppins', color: 'var(--muted)' }}>
        <div className="live-dot" style={{ display: 'inline-block', marginRight: 10 }} /> Loading article...
      </div>
    )
  }
