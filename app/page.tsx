...
function CryptoSection({ coins }: { coins: any[] }) {
  return (
    <FadeInSection>
      <section style={{ marginBottom: '3rem' }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <table className="crypto-table">
            ...
            <tbody>
              {coins.map(c => (
                <tr key={c.slug} style={{ cursor: 'pointer' }}>
                  <td style={{ color: 'var(--muted)', fontWeight: 700 }}>{c.market_cap_rank}</td>
                  <td>
                    <Link href={`/crypto/${c.slug}`} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
                      {c.image && <img src={c.image} alt={c.name} width={30} height={30} style={{ borderRadius: '50%', flexShrink: 0 }} />}
                      <div>
                        <div style={{ fontWeight: 700, fontSize: '14px', color: 'var(--text)' }}>{c.name}</div>
                        <div style={{ color: 'var(--muted)', fontSize: '11px', fontWeight: 600 }}>{c.symbol}</div>
                      </div>
                    </Link>
                  </td>
                  <td style={{ textAlign: 'right', fontFamily: '"JetBrains Mono"', fontWeight: 700, fontSize: '14px', color: 'var(--text)' }}>
                    <Link href={`/crypto/${c.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      ${formatPrice(c.price)}
                    </Link>
                  </td>
...
function NewsSection({ articles }: { articles: any[] }) {
  return (
    <section style={{ marginBottom: '3rem' }}>
      <SectionHeader title="📰 Latest Tech News" href="/news" accent="news" />
      <StaggerGrid style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: '1rem' }}>
        {articles.map((item) => (
            ...
            <Link href={`/news/${item.slug}`} style={{ textDecoration: 'none' }}>
              <article className="card card-news accent-news" style={{ height: '100%' }}>
                ...
                <div style={{ padding: '1rem' }}>
                  ...
                  <h3 style={{ fontFamily: 'Poppins', fontWeight: 700, fontSize: '14px', lineHeight: 1.4, color: 'var(--text)', marginBottom: '0.4rem' }}>{truncate(item.title, 80)}</h3>
...
function GitHubSection({ repos }: { repos: any[] }) {
  return (
    <section style={{ marginBottom: '3rem' }}>
      ...
      <StaggerGrid style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: '1rem' }}>
        {repos.map(repo => (
          <StaggerItem key={repo.slug}>
            <Link href={`/github/${repo.slug}`} style={{ textDecoration: 'none' }}>
...
function AIToolsSidebar({ tools }: { tools: any[] }) {
  return (
      ...
      <StaggerGrid>
        {tools.map(tool => (
          <StaggerItem key={tool.slug}>
            <Link href={`/ai-tools/${tool.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: '0.75rem' }}>
...
function JobsSidebar({ jobs }: { jobs: any[] }) {
  return (
    <div style={{ marginBottom: '2rem' }}>
      ...
      <StaggerGrid>
        {jobs.map(job => (
          <StaggerItem key={job.slug}>
            <Link href={`/jobs/${job.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: '0.75rem' }}>
...
function StartupsSidebar({ startups }: { startups: any[] }) {
  return (
    <div style={{ marginBottom: '2rem' }}>
      ...
      <StaggerGrid>
        {startups.map(item => (
          <StaggerItem key={item.slug}>
            <Link href={`/startups/${item.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: '0.75rem' }}>
